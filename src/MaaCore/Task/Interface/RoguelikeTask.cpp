...
bool asst::RoguelikeTask::set_params(const json::value& params)
{
    ...
    m_roguelike_config_ptr->set_only_start_with_elite_two(
        params.get("only_start_with_elite_two", false));
    // 是否凹开局远见密文板
    ...
    // 是否跳过阵容完备度检测
    m_roguelike_config_ptr->set_recruitment_team_complete(
        params.get("skip_team_completeness_check", false));
    ...
    m_roguelike_config_ptr->set_use_foldartal(
