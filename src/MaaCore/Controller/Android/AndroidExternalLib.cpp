...
{
    return m_handle != nullptr && m_loaded && m_funcs.get_locked_pixels != nullptr;
}
...
FrameInfo AndroidExternalLib::GetLockedPixels(void) const
{
    ...
    LogDebug << "LockedPixels - width: " << result.width << ", height: " << result.height
             << ", stride: " << result.stride << ", length: " << result.length << ", data: " << result.data
...
int AndroidExternalLib::UnlockPixels(FrameInfo info) const
{
    ...
    LogDebug << "Unlocking pixels - data: " << info.data << ", frame_ref: " << info.frame_ref;
...
int AndroidExternalLib::DispatchInputMessage(MethodParam param) const
{
    ...
    switch (param.method) {
    case START_GAME:
        LogDebug << "START_GAME - package_name: "
                 << (param.args.start_game.package_name ? param.args.start_game.package_name : "null")
                 << ", force_stop: " << param.args.start_game.force_stop;
        break;
    case STOP_GAME:
        LogDebug << "STOP_GAME - client_type: "
                 << (param.args.stop_game.client_type ? param.args.stop_game.client_type : "null");
        break;
    case INPUT:
